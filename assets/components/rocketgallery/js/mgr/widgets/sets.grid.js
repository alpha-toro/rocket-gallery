RocketGallery.grid.Sets = function(config) {
    config = config || {};
    var gridView = new Ext.grid.GridView({
        ...
        ,getRowClass : function (row, index) {
            ...
            if (data.published == 0) {
                cls = 'rocketgallery-grid-set-unpublished'; // Faded color
            }
    ...
    Ext.applyIf(config,{
        ...
        ,tbar: [{
            text: _('rocketgallery.set_create')
            ,handler: this.createSet
            ,scope: this
        }]
    });
    RocketGallery.grid.Sets.superclass.constructor.call(this,config);
    this.addEvents('sort');
    this.on('sort',this.onSort,this);
};
Ext.extend(RocketGallery.grid.Sets,MODx.grid.Grid,{
    windows: {}
    ...
    ,getMenu: function() {
        ...
        var m = [];
        m.push({
            text: _('rocketgallery.set_manage')
            ,handler: this.manageSet
        });
        m.push({
            text: _('rocketgallery.set_update')
            ,handler: this.updateSet
        });

        if (record.published) {
            m.push({
                text: _('rocketgallery.unpublish')
                ,handler: this.unpublishSet
            });

        } else {
            m.push({
                text: _('rocketgallery.publish')
        ...
        m.push('-');
        m.push({
            text: _('rocketgallery.set_remove')
    ...
    ,createSet: function(btn,e) {
        ...
        if (!this.windows.createSet) {
            this.windows.createSet = MODx.load({
                xtype: 'rocketgallery-window-set-create'
                ,listeners: {
    ...
    ,updateSet: function(btn,e) {
        ...
        if (!this.windows.updateSet) {
            this.windows.updateSet = MODx.load({
                xtype: 'rocketgallery-window-set-update'
                ,record: r
                ,listeners: {
                    'success': {fn:function() {this.refresh();},scope:this}
                }
    ...
    ,removeSet: function(btn,e) {
        ...
        MODx.msg.confirm({
            title: _('rocketgallery.set_remove')
            ,text: _('rocketgallery.set_remove_confirm')
            ,url: this.config.url
            ,params: {
...
Ext.reg('rocketgallery-grid-sets',RocketGallery.grid.Sets);


RocketGallery.window.CreateSet = function(config) {
    config = config || {};
    this.ident = config.ident || 'mecset'+Ext.id();
    Ext.applyIf(config,{
        title: _('rocketgallery.set_create')
        ,id: this.ident
        ,autoHeight: true
        ,width: 475
        ,modal: true
    ...
    RocketGallery.window.CreateSet.superclass.constructor.call(this,config);
};
Ext.extend(RocketGallery.window.CreateSet,MODx.Window);
Ext.reg('rocketgallery-window-set-create',RocketGallery.window.CreateSet);

RocketGallery.window.UpdateSet = function(config) {
    config = config || {};
    this.ident = config.ident || 'meuset'+Ext.id();
    Ext.applyIf(config,{
        title: _('rocketgallery.set_update')
        ,id: this.ident
        ,autoHeight: true
        ,width: 475
        ,modal: true
    ...
    RocketGallery.window.UpdateSet.superclass.constructor.call(this,config);
};
Ext.extend(RocketGallery.window.UpdateSet,MODx.Window);
Ext.reg('rocketgallery-window-set-update',RocketGallery.window.UpdateSet);
